...
let isSyncing = false;
let pendingReceiptUploads = []; // {expenseRowId, base64, filename} アップロード待ちの領収書
...
function logout() {
  sessionStorage.removeItem('rcl_session');
  location.reload();
}
...
function initApp() {
  ...
  fetchFromServer(); // バックグラウンドでGASから最新データ取得
}

// ============================================
// ローカルキャッシュ（オフライン時・即時表示用）
// サーバーが正なので、起動時は必ずfetchFromServerで上書きする
// ============================================
function loadLocalCache() {
  try {
    const cached = JSON.parse(localStorage.getItem(LS_KEY));
    if (cached) db = Object.assign(db, cached);
  } catch (e) { /* ignore */ }
}
...
function setSyncBadge(state) {
  ...
  if (state === 'syncing') {
    ...
    b.innerHTML = '<i class="ti ti-cloud-upload"></i> 同期中…';
  } else if (state === 'ok') {
    b.className = 'sync-badge';
    b.innerHTML = '<i class="ti ti-cloud-check"></i> 同期済み';
  } else {
    b.className = 'sync-badge';
    b.innerHTML = '<i class="ti ti-cloud-off"></i> オフライン';
  }
}
...
async function fetchFromServer() {
  ...
  try {
    ...
    console.error(e);
    setSyncBadge('err');
    showToast('サーバーに接続できません。オフラインのデータを表示しています', 'toast', 'err');
  }
}
...
async function pushToServer(action, payload) {
  setSyncBadge('syncing');
  try {
    await gasCall(action, payload);
    setSyncBadge('ok');
    return true;
  } catch (e) {
    console.error(e);
    setSyncBadge('err');
    showToast('保存に失敗しました。通信environmentを確認してください', 'toast', 'err');
    return false;
  }
}
...
function bizName(id) {
  const b = db.bizList.find(x => x.id === id);
  return b ? b.name : id;
}
...
function renderBizSelects() {
  ...
  // 経費行内のセレクトも更新
  document.querySelectorAll('[id^="ebiz-"]').forEach(sel => {
    const prev = sel.value;
    sel.innerHTML = opts;
...
function addExtraSales() {
  ...
  div.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
      ...
      <button class="delete-btn" onclick="removeExtraSales(${id})"><i class="ti ti-x"></i></button>
    </div>
    ...
    <div>
      ...
      <input type="number" id="esamt-${id}" placeholder="0" min="0" oninput="updateSalesTotal()" />
    </div>`;
  document.getElementById('extra-sales-list').appendChild(div);
}
function removeExtraSales(id) {
  document.getElementById('es-' + id)?.remove();
  extraSalesIds = extraSalesIds.filter(x => x !== id);
  updateSalesTotal();
}
...
function onAccountChange(id) {
  document.getElementById('ecustom-wrap-' + id).style.display = document.getElementById('eacc-' + id).value === CUSTOM_LABEL ? 'block' : 'none';
}
...
function addExpenseRow() {
  ...
  div.innerHTML = `
    <div class="exp-row-head">
      <span style="font-size:12px;font-weight:600;color:var(--text-sub);">経費</span>
      <button class="delete-btn" onclick="removeExpense(${id})"><i class="ti ti-x"></i></button>
    </div>
    <div class="row2" style="margin-bottom:8px;">
      <div>
        <label class="form-label">科目</label>
        <select id="eacc-${id}" onchange="onAccountChange(${id})">${buildAccountOptions()}</select>
        <div id="ecustom-wrap-${id}" style="display:none;margin-top:6px;">
          <input type="text" id="ecustom-${id}" placeholder="科目名を入力" />
        </div>
      </div>
    ...
    <div>
      ...
      <div class="receipt-attach" id="receipt-wrap-${id}">
        <button type="button" class="receipt-add-btn" onclick="document.getElementById('receipt-file-${id}').click()">
          <i class="ti ti-camera"></i>写真を追加
        </button>
      </div>
...
// ── 領収書：圧縮してbase64化（アップロードはsaveEntry時にまとめて行う） ──
function handleReceiptFile(id, file) {
  ...
  reader.onload = e => {
    ...
    img.onload = () => {
      ...
      renderReceiptThumb(id, dataUrl, file.name || 'receipt.jpg');
    };
...
function renderReceiptThumb(id, dataUrl, filename) {
  ...
  wrap.dataset.image = dataUrl;
  wrap.dataset.filename = filename;
  wrap.innerHTML = `
    <img src="${dataUrl}" class="receipt-thumb" onclick="openLightbox('${dataUrl}')" alt="領収書サムネイル" />
    <span class="receipt-badge"><i class="ti ti-check" style="font-size:9px;"></i> 添付済み（保存時にドライブへアップロード）</span>
...
async function saveEntry() {
  ...
  // 経費：領収書があればドライブにアップロードしてからURLを保存
  const expensesRaw = expenseIds.map(id => {
    const wrap = document.getElementById('receipt-wrap-' + id);
    return {
      id,
      account: getAccountValue(id),
      payment: document.getElementById('epay-' + id)?.value || '現金',
  ...
  try {
    ...
    for (const ex of expensesRaw) {
      ...
      if (ex.receiptDataUrl) {
        const uploadRes = await gasCall('uploadReceipt', {
          date, filename: ex.receiptFilename, base64: ex.receiptDataUrl.split(',')[1]
        });
    ...
    console.error(e);
    showToast('保存中にエラーが発生しました', 'toast', 'err');
  } finally {
...
function calcKyuyo(salary, shakai) {
  const honnin = half(shakai);
  const kaisha = shakai - honnin;
  const tedori = salary - honnin;
...
async function saveKyuyoSettings() {
  ...
  db.kyuyoSettings = { salary, shakai };
  saveLocalCache();
  await pushToServer('saveKyuyoSettings', db.kyuyoSettings);
  showToast('設定を保存しました', 'kyuyo-toast');
  updateKyuyoPreview();
}
...
async function executeImport() {
  ...
  try {
    for (const r of pendingImport) {
      ...
      let entry;
      if (idx >= 0) {
        db.entries[idx].salesCash = r.cash;
        db.entries[idx].salesCashBiz = biz;
        db.entries[idx].extraSales = extraSales;
        if (r.memo) db.entries[idx].memo = r.memo;
        entry = db.entries[idx];
        updated++;
      } else {
        entry = { date: r.date, salesCash: r.cash, salesCashBiz: biz, extraSales, expenses: [], memo: r.memo };
...
// 月報
// ============================================
function filterByBiz(list, getBiz, bizFilter) {
  if (bizFilter === 'all' || !bizFilter) return list;
  return list.filter(x => getBiz(x) === bizFilter);
}
...
function renderGeppo() {
  ...
  entries.forEach(e => {
    if (e.salesCash > 0 && (bizFilter === 'all' || e.salesCashBiz === bizFilter)) {
      ts += e.salesCash;
      salesBreak['現金売上'] = (salesBreak['現金売上'] || 0) + e.salesCash;
...
function renderBizList() {
  ...
  if (!list) return;
  list.innerHTML = db.bizList.map((b, i) => `
    <div class="biz-mgmt-row">
      <span class="biz-tag ${bizTagClass(b.id)}">${b.name}</span>
      <input type="text" value="${b.name}" onchange="renameBiz(${i}, this.value)" />
      <button class="delete-btn" onclick="removeBiz(${i})"><i class="ti ti-x"></i></button>
    </div>`).join('');
}
async function renameBiz(i, name) {
  if (!name.trim()) return;
  db.bizList[i].name = name.trim();
  saveLocalCache();
  await pushToServer('saveBizList', db.bizList);
  renderBizSelects(); renderBizList();
  showToast('事業区分を更新しました', 'toast2');
}
async function removeBiz(i) {
  if (db.bizList.length <= 1) { showToast('最低1つは必要です', 'toast2', 'err'); return; }
  if (!confirm(db.bizList[i].name + ' を削除しますか？（過去データの区分は残ります）')) return;
  db.bizList.splice(i, 1);
  saveLocalCache();
  await pushToServer('saveBizList', db.bizList);
  renderBizSelects(); renderBizList();
}
async function addBiz() {
  const name = prompt('事業区分名を入力してください（例: 物販、コンサル業）');
  if (name && name.trim()) {
    const id = 'biz_' + Date.now();
    db.bizList.push({ id, name: name.trim() });
    saveLocalCache();
    await pushToServer('saveBizList', db.bizList);
    renderBizSelects(); renderBizList();
    showToast('追加しました', 'toast2');
  }
}
...
function renderAccountList() {
  ...
  if (!list) return;
  list.innerHTML = db.accounts.map((a, i) => `
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
      <span style="flex:1;font-size:13px;">${a}</span>
      <button class="delete-btn" onclick="removeAccount(${i})"><i class="ti ti-x"></i></button>
    </div>`).join('') + `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;opacity:0.45;"><span style="flex:1;font-size:13px;font-style:italic;">${CUSTOM_LABEL}（固定）</span></div>`;
}
async function addAccount() {
  const name = prompt('科目名を入力してください');
  if (name && name.trim()) {
    db.accounts.push(name.trim());
    saveLocalCache();
    await pushToServer('saveAccounts', db.accounts);
    renderAccountList();
    showToast('追加しました', 'toast2');
  }
}
async function removeAccount(i) {
  if (!confirm(db.accounts[i] + ' を削除しますか？')) return;
...
function renderStorageInfo() {
  ...
  document.getElementById('storage-info').innerHTML = `
    ...
    <div style="font-size:11px;color:var(--text-sub);margin-top:6px;">※ Googleドライブの容量に依存するため、アプリ側の重さは気にせず使えます</div>`;
}
...
function exportCSV(type) {
  ...
  if (type === 'daily') {
    ...
    csv = '日付,事業区分,借方科目,貸方科目,金額,摘要,支払手段\n';
    if (kyuyo && bizFilter === 'all') {
      csv += `${month}-01,全社共通,役員報酬,預り金,${kyuyo.honnin},社保本人負担分,\n`;
      csv += `${month}-01,全社共通,役員報酬,普通預金,${kyuyo.tedori},役員報酬手取振込,普通預金\n`;
      csv += `${month}-01,全社共通,法定福利費,普通預金,${kyuyo.kaisha},社保会社負担分,普通預金\n`;
      csv += `${month}-01,全社共通,預り金,普通預金,${kyuyo.honnin},社保本人分納付,普通預金\n`;
    }
    entries.forEach(e => {
      if (e.salesCash > 0 && (bizFilter === 'all' || e.salesCashBiz === bizFilter)) csv += `${e.date},${bizName(e.salesCashBiz)},現金,売上,${e.salesCash},現金売上,現金\n`;
      (e.extraSales || []).forEach(es => { if (bizFilter === 'all' || es.biz === bizFilter) csv += `${e.date},${bizName(es.biz)},売掛金,売上,${es.amount},"${es.name}",\n`; });
      (e.expenses || []).forEach(ex => { if (bizFilter === 'all' || ex.biz === bizFilter) csv += `${e.date},${bizName(ex.biz)},${ex.account},${ex.payment},${ex.amount},"${ex.desc || ''}",${ex.payment}\n`; });
    });
    filename = `仕訳帳${bizSuffix}_${month}.csv`;
  } else {
    csv = '日付,事業区分,種別,名称,支払手段,金額,摘要,領収書URL\n';
    if (kyuyo && bizFilter === 'all') {
      csv += `${month}-01,全社共通,給与,役員報酬,,${kyuyo.salary},,\n`;
      csv += `${month}-01,全社共通,給与,社会保険（会社負担）,普通預金,${kyuyo.kaisha},,\n`;
      csv += `${month}-01,全社共通,給与,社会保険（本人負担）,預り金,${kyuyo.honnin},,\n`;
    }
    entries.forEach(e => {
      if (e.salesCash > 0 && (bizFilter === 'all' || e.salesCashBiz === bizFilter)) csv += `${e.date},${bizName(e.salesCashBiz)},売上,現金売上,現金,${e.salesCash},"",\n`;
      (e.extraSales || []).forEach(es => { if (bizFilter === 'all' || es.biz === bizFilter) csv += `${e.date},${bizName(es.biz)},売上,"${es.name}",,${es.amount},"",\n`; });
      (e.expenses || []).forEach(ex => { if (bizFilter === 'all' || ex.biz === bizFilter) csv += `${e.date},${bizName(ex.biz)},経費,${ex.account},${ex.payment},${ex.amount},"${ex.desc || ''}",${ex.receiptUrl || ''}\n`; });
    });
...
async function clearAllData() {
  if (!confirm('全データを削除します。この操作はサーバー上のデータも削除し、元に戻せません。よろしいですか？')) return;
  if (!confirm('本当によろしいですか？最終確認です。')) return;
  document.getElementById('loading-overlay').classList.add('show');
  try {
    await pushToServer('clearAll', {});
    db = { entries: [], kyuyo: [], accounts: [...DEF_ACCOUNTS], bizList: [...DEF_BIZ], kyuyoSettings: { salary: 45000, shakai: 23124 } };
    saveLocalCache();
    showToast('リセットしました', 'toast2');
    populateMonthSelects();
    renderBizSelects(); renderAccountList(); renderBizList();
  } finally {
    document.getElementById('loading-overlay').classList.remove('show');
  }
}
...
async function onMeisaiFileSelect(file) {
  ...
  try {
    ...
    for (const item of result.items) {
      // 同じIDが既にあればスキップ（重複取込防止）
      if (!db.meisai.find(m => m.id === item.id)) {
        db.meisai.push(item);
        added++;
      } else {
        skipped++;
...
function updateMeisaiBadge() {
  ...
  if (banner && text) {
    if (unchecked > 0) {
      ...
      text.textContent = `未確認の明細が ${unchecked} 件あります。内容を確認してチェックしてください。`;
    } else {
...
function renderMeisaiList() {
  if (!db.meisai || !db.meisai.length) {
    document.getElementById('meisai-list-area').style.display = 'none';
    return;
  }
  ...
  tbody.innerHTML = items.map(item => {
    ...
    const rowClass = item.checked ? 'checked-row' : '';
    const accountOpts = [...(db.accounts || []), 'その他経費'].map(a =>
      `<option value="${a}" ${item.debit === a ? 'selected' : ''}>${a}</option>`
    ).join('');
    // 貸方のよく使う選択肢
    const creditOpts = ['JCBカード', '出光法人カード', '普通預金（ゆうちょ）', '普通預金（あおぞら）', '現金', '役員報酬', '受取利息', '雑収入', '仮受金', 'その他'].map(a =>
      `<option value="${a}" ${item.credit === a ? 'selected' : ''}>${a}</option>`
    ).join('');

    return `<tr class="${rowClass}" id="mrow-${item.id}">
      <td style="text-align:center;">
        <input type="checkbox" ${item.checked ? 'checked' : ''} onchange="toggleMeisaiCheck('${item.id}', this.checked)" />
      </td>
      ...
      <td>
        <select onchange="updateMeisaiField('${item.id}','debit',this.value)">${accountOpts}</select>
      </td>
      <td>
        <select onchange="updateMeisaiField('${item.id}','credit',this.value)">${creditOpts}</select>
      </td>
      ...
      <td>
        <input type="text" value="${item.memo || ''}" placeholder="品目・内容を入力" onchange="updateMeisaiField('${item.id}','memo',this.value)" onblur="updateMeisaiField('${item.id}','memo',this.value)" />
      </td>
    </tr>`;
  }).join('');
  ...
  document.getElementById('meisai-list-footer').innerHTML =
    `表示: ${items.length}件 / 合計: ¥${total.toLocaleString()} ／ <span class="unchecked-badge">未確認 ${unchecked}件</span>`;

  updateMeisaiBadge();
}

// ── フィールド更新 ──
function updateMeisaiField(id, field, value) {
  const item = db.meisai.find(m => m.id === id);
  if (!item) return;
  item[field] = value;
  saveLocalCache();
}
...
function toggleMeisaiCheck(id, checked) {
  ...
  if (row) row.className = checked ? 'checked-row' : '';
  saveLocalCache();
  updateMeisaiBadge();
  document.getElementById('meisai-list-footer').innerHTML =
    (() => {
      ...
      if (filterChecked === 'checked') items = items.filter(m => m.checked);
      const total = items.reduce((s, m) => s + m.amount, 0);
      const unc = items.filter(m => !m.checked).length;
      return `表示: ${items.length}件 / 合計: ¥${total.toLocaleString()} ／ <span class="unchecked-badge">未確認 ${unc}件</span>`;
    })();
}
...
function bulkCheckAll() {
  ...
  const filterSource = document.getElementById('meisai-filter-source').value;
  db.meisai.forEach(m => {
    if (filterMonth !== 'all' && !m.date.startsWith(filterMonth)) return;
    if (filterSource !== 'all' && m.source !== filterSource) return;
    m.checked = true;
  });
  saveLocalCache();
  renderMeisaiList();
...
function bulkUncheckAll() {
  db.meisai.forEach(m => m.checked = false);
  saveLocalCache();
  renderMeisaiList();
}
...
async function saveMeisaiToServer() {
  document.getElementById('loading-overlay').classList.add('show');
  try {
    const ok = await pushToServer('saveMeisai', db.meisai);
    if (ok) showMeisaiToast('明細データを保存しました ✓');
  } finally {
    document.getElementById('loading-overlay').classList.remove('show');
  }
}
...
// ── 仕訳帳に明細データも表示 ──
const _origRenderLedger = renderLedger;
function renderLedger() {
  _origRenderLedger();
  // 明細データを仕訳帳に追加表示
  const month = document.getElementById('ledger-month').value;
  const bizFilter = document.getElementById('ledger-biz').value || 'all';
  if (bizFilter !== 'all') return; // 事業フィルタ時はスキップ

  const meisaiItems = (db.meisai || []).filter(m => m.date.startsWith(month) && m.checked);
  if (!meisaiItems.length) return;

  const c = document.getElementById('ledger-entries');
  let html = c.innerHTML;
  if (html.includes('データなし')) html = '';
  ...
  meisaiItems.forEach(m => {
    ...
    html += `<div class="ledger-row">
      <div>
        <div style="font-weight:600;">${m.date.substring(5)} <span class="source-badge ${srcClass}">${m.source}</span> ${m.debit}</div>
        <div class="ledger-date">${m.shopName}${m.memo ? ' / ' + m.memo : ''} → 貸方: ${m.credit}</div>
      </div>
      <span class="red">¥${m.amount.toLocaleString()}</span>
    </div>`;
  });
  c.innerHTML = html;
}
...
function exportCSV(type) {
  if (type !== 'jizuke' && type !== 'all') { _origExportCSV(type); return; }

  const month = document.getElementById('export-month').value;
  const bizFilter = document.getElementById('export-biz').value || 'all';
  const meisaiItems = (db.meisai || []).filter(m => m.date.startsWith(month) && m.checked);

  if (!meisaiItems.length || bizFilter !== 'all') { _origExportCSV(type); return; }

  // 元のCSV取得（一時的にexport-bizをallにして）
  const entries = db.entries.filter(e => e.date.startsWith(month));
  const kyuyo = db.kyuyo.find(k => k.month === month);
  let csv = '', filename = '';
  ...
  if (type === 'jizuke') {
    ...
    filename = `仕訳帳_${month}.csv`;
  } else {
    // 全データ
    csv = '日付,区分,口座,借方科目,貸方科目,金額,利用先,摘要\n';
    meisaiItems.forEach(m => {
      csv += `${m.date},銀行明細,${m.source},${m.debit},${m.credit},${m.amount},"${m.shopName}","${m.memo || ''}"\n`;
    });
    entries.forEach(e => {
      if (e.salesCash > 0) csv += `${e.date},売上,,現金,売上,${e.salesCash},現金売上,\n`;
      (e.expenses || []).forEach(ex => { csv += `${e.date},経費,,${ex.account},${ex.payment},${ex.amount},,${ex.desc || ''}\n`; });
    });
    filename = `全データ_${month}.csv`;
  }
