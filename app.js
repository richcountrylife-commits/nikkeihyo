...
async function gasCall(action, payload) {
  ...
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, redirect: 'follow', body });
...
function updateSalesTotal() {
  ...
  const extras = extraSalesIds.reduce((s, id) => s + (parseFloat(document.getElementById('esamt-' + id) && document.getElementById('esamt-' + id).value) || 0), 0);
...
function addExtraSales() {
  ...
  div.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
    ...
    '<div><label class="form-label">事業区分</label><select id="esbiz-' + id + '">' + bizOpts + '</select></div></div>' +
...
function removeExtraSales(id) {
  document.getElementById('es-' + id) && document.getElementById('es-' + id).remove();
  extraSalesIds = extraSalesIds.filter(x => x !== id); updateSalesTotal();
}
...
function onAccountChange(id) {
  const wrap = document.getElementById('ecustom-wrap-' + id);
  const sel = document.getElementById('eacc-' + id);
  if (wrap && sel) wrap.style.display = sel.value === CUSTOM_LABEL ? 'block' : 'none';
}
function getAccountValue(id) {
  const sel = document.getElementById('eacc-' + id);
  if (!sel) return 'その他経費';
  if (sel.value === CUSTOM_LABEL) { const c = document.getElementById('ecustom-' + id); return (c && c.value.trim()) || 'その他経費'; }
  return sel.value;
}
...
function addExpenseRow() {
  ...
  div.innerHTML = '<div class="exp-row-head"><span style="font-size:12px;font-weight:600;color:var(--text-sub);">経費</span>' +
    ...
    '<div id="ecustom-wrap-' + id + '" style="display:none;margin-top:6px;"><input type="text" id="ecustom-' + id + '" placeholder="科目名を入力" /></div></div>' +
    '<div><label class="form-label">事業区分</label><select id="ebiz-' + id + '">' + bizOpts + '</select></div></div>' +
    '<div class="row2" style="margin-bottom:8px;">' +
    '<div><label class="form-label">支払手段</label><select id="epay-' + id + '">' + PAYMENTS.map(p => '<option>' + p + '</option>').join('') + '</select></div>' +
    '<div><label class="form-label">金額（円）</label><input type="number" id="eamt-' + id + '" placeholder="0" min="0" /></div></div>' +
    ...
    '<button type="button" class="receipt-add-btn" onclick="document.getElementById(\'receipt-file-' + id + '\').click()"><i class="ti ti-camera"></i>写真を追加</button></div>' +
    '<input type="file" id="receipt-file-' + id + '" accept="image/*" capture="environment" style="display:none;" onchange="handleReceiptFile(' + id + ', this.files[0])" /></div>';
  document.getElementById('expense-list').appendChild(div);
}
function removeExpense(id) {
  document.getElementById('exp-' + id) && document.getElementById('exp-' + id).remove();
...
async function saveEntry() {
  ...
  const extraSales = extraSalesIds.map(id => ({
    name: (document.getElementById('esname-' + id) && document.getElementById('esname-' + id).value) || 'その他売上',
    amount: parseFloat(document.getElementById('esamt-' + id) && document.getElementById('esamt-' + id).value) || 0,
    biz: (document.getElementById('esbiz-' + id) && document.getElementById('esbiz-' + id).value) || defaultBiz
  })).filter(e => e.amount > 0);
  const expensesRaw = expenseIds.map(id => {
    const wrap = document.getElementById('receipt-wrap-' + id);
    return {
      id, account: getAccountValue(id),
      payment: (document.getElementById('epay-' + id) && document.getElementById('epay-' + id).value) || '現金',
      amount: parseFloat(document.getElementById('eamt-' + id) && document.getElementById('eamt-' + id).value) || 0,
      desc: (document.getElementById('edesc-' + id) && document.getElementById('edesc-' + id).value) || '',
      biz: (document.getElementById('ebiz-' + id) && document.getElementById('ebiz-' + id).value) || defaultBiz,
  ...
  try {
    ...
    for (const ex of expensesRaw) {
      ...
      if (ex.receiptDataUrl) {
        ...
        } catch(e) { console.error('領収書アップロード失敗:', e); }
      }
    ...
    extraSalesIds = []; expenseIds = []; updateSalesTotal(); onDateChange();
...
function editEntry(date) {
  ...
  if (bizSel && entry.salesCashBiz) bizSel.value = entry.salesCashBiz;
...
async function deleteEntry(date) {
  ...
  try {
    ...
    populateMonthSelects(); onDateChange();
  } catch(e) { showToast('削除に失敗しました', 'toast', 'err'); }
  finally { document.getElementById('loading-overlay').classList.remove('show'); }
}
...
function parseCSV(text, filename) {
  ...
  if (tbody) tbody.innerHTML = rows.filter(r => r.cash + r.examt > 0).slice(0, 10).map(r =>
...
async function executeImport() {
  const bizEl = document.getElementById('import-biz');
  const biz = (bizEl && bizEl.value) || (db.bizList[0] && db.bizList[0].id);
...
function renderGeppo() {
  const monthEl = document.getElementById('geppo-month');
  const bizEl = document.getElementById('geppo-biz');
  if (!monthEl) return;
  const month = monthEl.value;
  const bizFilter = (bizEl && bizEl.value) || 'all';
  ...
  setEl('g-sales', fmt(ts));
  setEl('g-expense', fmt(te));
  setEl('g-profit', fmt(ts - te));
  setEl('g-kyuyo', kyuyo ? fmt(kyuyo.salary) : '未記録');
  setEl('g-days', entries.length + '日');

  const bb = document.getElementById('g-biz-breakdown');
  const bbe = Object.entries(bizBreak).filter(([,v]) => v > 0);
  if (bb) bb.innerHTML = bbe.length ? bbe.map(([k,v]) => '<div class="ledger-row"><span class="biz-tag ' + bizTagClass(k) + '">' + bizName(k) + '</span><span class="green" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">売上なし</div>';

  const sb = document.getElementById('g-sales-breakdown');
  const sbe = Object.entries(salesBreak).filter(([,v]) => v > 0);
  if (sb) sb.innerHTML = sbe.length ? sbe.map(([k,v]) => '<div class="ledger-row"><span>' + k + '</span><span class="green" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">売上なし</div>';

  const pb = document.getElementById('g-payment-breakdown');
  const pbe = Object.entries(payBreak).filter(([,v]) => v > 0);
  if (pb) pb.innerHTML = pbe.length ? pbe.map(([k,v]) => '<div class="ledger-row"><span>' + k + '</span><span class="red" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">経費なし</div>';

  const dl = document.getElementById('g-daily-list');
  if (!dl) return;
  if (!entries.length) { dl.innerHTML = '<div class="empty-state">データなし</div>'; return; }
  dl.innerHTML = '<table class="dt"><thead><tr><th>日</th><th class="amt">売上</th><th class="amt">経費</th><th class="amt">粗利</th></tr></thead><tbody>' +
    entries.map(e => {
      const biz = bizFilter;
      const s = (biz === 'all' || e.salesCashBiz === biz ? e.salesCash : 0) + (e.extraSales || []).filter(x => biz === 'all' || x.biz === biz).reduce((a,x) => a+x.amount, 0);
      const ex = (e.expenses || []).filter(x => biz === 'all' || x.biz === biz).reduce((a,x) => a+x.amount, 0);
      const pr = s - ex;
...
function renderLedger() {
  ...
  const bizEl = document.getElementById('ledger-biz');
  if (!monthEl) return;
  const month = monthEl.value;
  const bizFilter = (bizEl && bizEl.value) || 'all';
  const entries = db.entries.filter(e => e.date.startsWith(month));
  ...
  entries.forEach(e => {
    if (e.salesCash > 0 && (bizFilter === 'all' || e.salesCashBiz === bizFilter)) {
      html += '<div class="ledger-row"><div><div style="font-weight:600;">' + e.date.substring(5) + ' 現金売上 <span class="biz-tag ' + bizTagClass(e.salesCashBiz) + '">' + bizName(e.salesCashBiz) + '</span></div>' + (e.memo ? '<div class="ledger-date">' + e.memo + '</div>' : '') + '</div>' +
        '<div style="display:flex;align-items:center;gap:8px;"><span class="green">' + fmt(e.salesCash) + '</span>' +
        '<button class="btn btn-outline" style="padding:3px 8px;font-size:10px;" onclick="editEntry(\'' + e.date + '\');switchTab(\'nikkkei\',document.querySelector(\'[data-tab=nikkkei]\'))"><i class="ti ti-pencil"></i></button></div></div>';
    }
    (e.extraSales || []).forEach(es => {
      if (bizFilter !== 'all' && es.biz !== bizFilter) return;
...
function renderBizList() {
  ...
  list.innerHTML = db.bizList.map((b, i) =>
    '<div class="biz-mgmt-row"><span class="biz-tag ' + bizTagClass(b.id) + '">' + b.name + '</span>' +
    '<input type="text" value="' + b.name + '" onchange="renameBiz(' + i + ', this.value)" />' +
    '<button class="delete-btn" onclick="removeBiz(' + i + ')"><i class="ti ti-x"></i></button></div>'
  ).join('');
}
...
function renderAccountList() {
  ...
  list.innerHTML = db.accounts.map((a, i) =>
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;"><span style="flex:1;font-size:13px;">' + a + '</span>' +
    '<button class="delete-btn" onclick="removeAccount(' + i + ')"><i class="ti ti-x"></i></button></div>'
  ).join('') + '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;opacity:0.45;"><span style="flex:1;font-size:13px;font-style:italic;">' + CUSTOM_LABEL + '（固定）</span></div>';
}
...
function renderStorageInfo() {
  ...
  el.innerHTML = '<div class="jizuke-row"><span>添付された領収書</span><span style="font-weight:600;">' + receiptCount + '枚</span></div>' +
...
function exportCSV(type) {
  ...
  const bizEl = document.getElementById('export-biz');
  if (!monthEl) return;
  const month = monthEl.value;
  const bizFilter = (bizEl && bizEl.value) || 'all';
  const entries = db.entries.filter(e => e.date.startsWith(month));
...
async function onMeisaiFileSelect(file) {
  ...
  try {
    ...
    const fileEl = document.getElementById('meisai-file');
...
function showMeisaiToast(msg, type) {
...
function renderMeisaiList() {
  if (!db.meisai || !db.meisai.length) {
    const area = document.getElementById('meisai-list-area');
    if (area) area.style.display = 'none'; return;
  }
  const area = document.getElementById('meisai-list-area');
  if (area) area.style.display = 'block';

  const filterMonthEl = document.getElementById('meisai-filter-month');
  const filterSourceEl = document.getElementById('meisai-filter-source');
  const filterCheckedEl = document.getElementById('meisai-filter-checked');
  const filterMonth = (filterMonthEl && filterMonthEl.value) || 'all';
  const filterSource = (filterSourceEl && filterSourceEl.value) || 'all';
  const filterChecked = (filterCheckedEl && filterCheckedEl.value) || 'all';

  const months = [...new Set(db.meisai.map(m => m.date.substring(0, 7)))].sort().reverse();
  if (filterMonthEl) {
    const prev = filterMonthEl.value;
    filterMonthEl.innerHTML = '<option value="all">全月</option>' + months.map(m => '<option value="' + m + '">' + m.replace('-','年') + '月</option>').join('');
    if (prev && months.includes(prev)) filterMonthEl.value = prev;
  }
  ...
  const footer = document.getElementById('meisai-list-footer');
  if (footer) footer.innerHTML = '表示: ' + items.length + '件 / 合計: ¥' + total.toLocaleString() + ' ／ <span class="unchecked-badge">未確認 ' + unchecked + '件</span>';
  updateMeisaiBadge();
}

function updateMeisaiField(id, field, value) {
  const item = db.meisai.find(m => m.id === id);
  if (!item) return; item[field] = value; saveLocalCache();
}
...
function bulkCheckAll() {
  const fm = (document.getElementById('meisai-filter-month') && document.getElementById('meisai-filter-month').value) || 'all';
  const fs = (document.getElementById('meisai-filter-source') && document.getElementById('meisai-filter-source').value) || 'all';
  db.meisai.forEach(m => {
    if (fm !== 'all' && !m.date.startsWith(fm)) return;
    if (fs !== 'all' && m.source !== fs) return;
    m.checked = true;
  });
...
function setGeppoMode(mode) {
  ...
  document.getElementById('g-daily-wrap').style.display = mode === 'month' ? 'block' : 'none';
...
function renderPeriodGeppo() {
  ...
  // 全体集計
  const total = calcGeppoForMonths(allMonths, bizFilter);

  // 表示更新
  const setEl = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
  setEl('g-sales', fmt(total.ts));
  setEl('g-expense', fmt(total.te));
  setEl('g-profit', fmt(total.ts - total.te));
  setEl('g-kyuyo', fmt(total.kyuyoTotal));
  setEl('g-days', total.days + '日');

  const bb = document.getElementById('g-biz-breakdown');
  const bbe = Object.entries(total.bizBreak).filter(([,v]) => v > 0);
  if (bb) bb.innerHTML = bbe.length ? bbe.map(([k,v]) => '<div class="ledger-row"><span class="biz-tag ' + bizTagClass(k) + '">' + bizName(k) + '</span><span class="green" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">売上なし</div>';

  const sb = document.getElementById('g-sales-breakdown');
  const sbe = Object.entries(total.salesBreak).filter(([,v]) => v > 0);
  if (sb) sb.innerHTML = sbe.length ? sbe.map(([k,v]) => '<div class="ledger-row"><span>' + k + '</span><span class="green" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">売上なし</div>';

  const pb = document.getElementById('g-payment-breakdown');
  const pbe = Object.entries(total.payBreak).filter(([,v]) => v > 0);
  if (pb) pb.innerHTML = pbe.length ? pbe.map(([k,v]) => '<div class="ledger-row"><span>' + k + '</span><span class="red" style="font-weight:600;">' + fmt(v) + '</span></div>').join('') : '<div class="empty-state">経費なし</div>';
  ...
  if (tbody) {
    tbody.innerHTML = allMonths.map(month => {
      const m = calcGeppoForMonths([month], bizFilter);
      const kyuyo = db.kyuyo.find(k => k.month === month);
      const pr = m.ts - m.te;
      return '<tr>' +
        '<td style="font-weight:600;">' + month.replace('-','年') + '月</td>' +
        '<td class="amt green">' + fmt(m.ts) + '</td>' +
        '<td class="amt red">' + fmt(m.te) + '</td>' +
        '<td class="amt ' + (pr >= 0 ? 'green' : 'red') + '">' + fmt(pr) + '</td>' +
        '<td class="amt blue">' + (kyuyo ? fmt(kyuyo.salary) : '-') + '</td>' +
        '</tr>';
    }).join('') +
    '<tr style="font-weight:700;border-top:2px solid var(--border-light);">' +
    '<td>合計</td>' +
    '<td class="amt green">' + fmt(total.ts) + '</td>' +
    '<td class="amt red">' + fmt(total.te) + '</td>' +
    '<td class="amt ' + (total.ts - total.te >= 0 ? 'green' : 'red') + '">' + fmt(total.ts - total.te) + '</td>' +
    '<td class="amt blue">' + fmt(total.kyuyoTotal) + '</td>' +
    '</tr>';
  }
}
